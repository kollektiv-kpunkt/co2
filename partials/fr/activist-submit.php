...
if (!$stmt->execute()) {
    ...
    if (in_array("some", $_POST["acti-type"])) {
        header("location: /wp-content/themes/co2/partials/fr/emails.php?fname={$fname}&lname={$lname}&to={$email}&email=acti-some&origin={$source}");
    } else {
        header("location: /wp-content/themes/co2/partials/fr/emails.php?fname={$fname}&lname={$lname}&to={$email}&email=acti&origin={$source}");
    }
}